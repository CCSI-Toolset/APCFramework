# A simple makefile for creating the CCSI APCFramework distribution
VERSION    := $(shell git describe --tags --dirty)
PRODUCT    := APC Framework
PROD_SNAME := APCFramework
LICENSE    := LICENSE.md
PKG_DIR    := CCSI_$(PROD_SNAME)_$(VERSION)
PACKAGE    := $(PKG_DIR).zip

PAYLOAD := APCFramework_Install.m \
     Examples \
     Templates \
     Toolbox \
     README.md \
